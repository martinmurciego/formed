var Formed = function(object, name, url, options, errors) {
  if (!options) { options = {}; }
  this.object = object;
  this.url = url;
...
Formed.prototype.error = function(name, options, tag) {
  var tag = tag || "div";
  if (!options) { options = {}; }
  ...
  if (this.errors[name]) {
    ...
    for (var i=0; i < this.errors[name].length; i++) {
      str = str + createErrorString(this.errors[name][i], tag, options) + "\n";
    };
